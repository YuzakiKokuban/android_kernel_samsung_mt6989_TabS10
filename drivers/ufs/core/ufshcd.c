...
static inline void ufshcd_utrl_clear(struct ufs_hba *hba, u32 mask)
{
	 ...
	 * corresponds to request slot 0. A bit in this field is set to 	 * by host software to indicate to the host controller that a transfer
	 * request slot is cleared. The host controller
	 * shall free up any resources associated to the request slot
	 * immediately, and shall set the associated bit in UTRLDBR to  The
	 * host software indicates no change to request slots by setting the
	 * associated bits in this field to  Bits in this field shall only
	 * be set or by host software when UTRLRSR is set to "
	 */
